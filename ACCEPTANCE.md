...
> 結論：**不過** — 閘門仍關閉因 **MILLION_USER_BAR #3 Partial**（15s UA 剪輯缺）；**#1 Pass**（INSTANT-CLARITY-GATE1-PASS）；**非**因變現 Blocked。P0②③／#7 已 **Pass**（DEVICE-THREE-GREEN Seeker）。**Production／soft-launch 仍禁止**。
...
| 結果 | 說明 |
|------|------|
| **不過** | P0①②③④ 已 **Pass**（P0②③＝**DEVICE-THREE-GREEN** Seeker 2026-09-21 Asia/Taipei；見 `docs/DEVICE_THREE_GREEN.md`）。閘門關閉因 MILLION_USER_BAR **#3 Partial**（非變現 Blocked）；**#1 Pass**（INSTANT-CLARITY-GATE1-PASS）。總評 **8 Pass／1 Partial／0 Fail** |
| 上架 | **禁止** Production／soft-launch 直到 #1–#9 全 Pass（現差 #3） |
...
（已補）**POUR-STREAM-THICK**已上 — `.pour-stream` **12→18px**／border-radius 9／較強 glow；`animatePour` half-width offset **7→9**；`.splash-particle` **6→7**＋win/complete/first splash 略濃（36/30/28）；KEEP POUR-FEEL-MIDLAND timing（POUR_MS=460／LAND_MS≈250）；`sync-www` → www／docs/play；accept `POUR-STREAM-THICK`；**#3 仍 Partial**（web `?ad=1` 加粗證據 ≠ 實機 paid UA；UA 凍結 — **不假標 Pass**）；**#1 仍 Partial**；閘門仍 **7 Pass／2 Partial／0 Fail**；**無** soft-arm／claim-juice／HUD pulse。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**

（已補）**INSTANT-CLARITY-P0**（V1＋C1＋T1）已上 — start ≤2.9s idle 擋→揭→倒；demote Blocked→Lid；thick gold lid；C1「Uncap the gold lid. Then pour.」＋sub＋Play；T1 L3 pain-then-joy（L1–2 不變）；`sync-www` → www／docs/play；accept `INSTANT-CLARITY-P0`；**#1 仍 Partial**（無外部 3s mute stranger 證據 — 不假 Pass）；**#3 仍 Partial**（POUR-FEEL-MIDLAND on main；15s UA clip 缺／UA 凍結）；閘門仍 **7 Pass／2 Partial／0 Fail**；**無** soft-arm。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**
...
（已補）**NATIVE-VC6-INTERNAL-SYNC**／**PLAY-PASTE-VC6-SYNC**已上 — docs 對齊 Play Active `1.0.5-internal-vc6-iapBusy`／vc6／1.0.5／**UNCAP-ONE-TAP**＋**IAP-PURCHASE-BUSY**；歷史 vc5／vc4／vc3／vc2；accept 兩項；**#7 仍 Fail**；閘門仍 5 Pass／3 Partial／1 Fail；**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**SHOP theme 金幣一鍵解鎖**已上 — Neon／Cat 等 theme 金幣 CTA 一鍵 `spendCoins`＋`unlockTheme`（同餘額檢查／toast）；hints-pack（120）／undo-level（80）仍兩次確認 toast；accept `SHOP-SPEND-CONFIRM` 已改；**無** soft-arm CSS；#9 維持 Partial（仍差 P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。
...
（已補）**CAP-TEACH-ARM**已上 — L3 `teach:cap` 首分鐘揭蓋教學：`capTeachDone` **僅**在真實 uncap 成功路徑寫入（`uncapTube`＋`persist`）；tip dismiss／howto 只隱藏、不清／不誤設 teach flag；load 後 once-per-load 首蓋管 `.lid-arm-nudge`～420ms＋`haptic('arm')`／`SFX.tap` @～300ms（reduced-motion 靜態光、不 spam）；uncap／`loadLevel` leave／`goHome` 清 timer／class；EN tip 改「Tap the gold lid」（UNCAP-ONE-TAP）；`sync-www` → www／docs/play；accept `CAP-TEACH-ARM`；#1 維持 Partial（產品側首分鐘 USP teach）；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
| C-揭蓋雙點 | 持液→有蓋＝shake＋「倒不進去」；無選取單點揭蓋（不占步）；undo 復蓋 | **Pass**（本輪邏輯） |
