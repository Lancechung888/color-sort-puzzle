...
> **Play Console developer account APPROVED** (2026-09-21 Asia/Taipei). **#7 remains Fail** until three green lights: (1) real AdMob app + interstitial/rewarded units wired, (2) `remove_ads` Play Billing device verify, (3) no mid-pour ads. Do **not** flip to Pass/Partial on account approval alone.
...
> **Play account approved** (2026-09-21 Asia/Taipei) — unlocks Console/AdMob create-app work. **#7 still Fail** until real AdMob units + `remove_ads` Billing device verify (three green lights). Do **not** mark #7 Pass/Partial on approval alone.
...
| 7 | 真 AdMob＋remove_ads | **Fail** | **Play 帳號已過審**（2026-09-21 Asia/Taipei）— 仍 **Fail**。AdMob **已**進 npm 依賴（`@capacitor-community/admob`）＋測單元／`USE_TEST_ADS` 已配線；Manifest `APPLICATION_ID` 可由 `scripts/patch-android-admob.sh` 補上。缺三綠燈：真 AdMob 單元配線、`remove_ads` Billing 實機驗證、倒水護欄實機確認。商店「去除廣告」→ **即將開放**，**不會**假授 `removeAds`（DEV flag 預設 OFF） |
