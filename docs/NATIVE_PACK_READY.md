...
> Marker: **NATIVE-VC5-INTERNAL-SYNC** · **NATIVE-VC6-LOCAL-AAB-SYNC** (current internal = `1.0.4-internal-vc5-uncap1tap` / vc5 / versionName **1.0.4** / Billing≥8 / prod AdMob / **UNCAP-ONE-TAP**; historical **NATIVE-VC3-INTERNAL-SYNC** = `1.0.2-internal-vc3-prodAdMob` / vc3; historical vc4 / 1.0.3; historical **NATIVE-INTERNAL-TESTING-SYNC** = `1.0.1-internal-vc2-testids`; #7 still Fail).  
...
> Honesty: Active Play AAB is still **vc5-uncap1tap** / **1.0.4** (built before **IAP-PURCHASE-BUSY** / #237). Tip `main` + script default = **ANDROID-VERSION-CODE-6** (`versionCode` **6** / `versionName` **1.0.5**) + **IAP-PURCHASE-BUSY**. Local signed AAB **already built** on this packaging box (2026-09-21 Asia/Taipei ~18:26): `/workspace/colortube-artifacts/ColorTubeSort-internal-20260921-1826-prodAdMob-vc6-iapBusy-release.aab` (+ `ColorTubeSort-internal-latest-release.aab` + `play-upload/ColorTubeSort-vc6-iapBusy.aab`) — **NOT uploaded** to Play; Active track remains vc5 until Console upload. Do **not** claim Play Active already equals vc6 / 1.0.5, or that IAP-PURCHASE-BUSY is in the Active vc5 AAB. Marker: **NATIVE-VC6-LOCAL-AAB-SYNC**.
...
| Play Active `versionCode` 5 / `versionName` 1.0.4 (`ANDROID-VERSION-CODE-5`) | **UPLOADED current** (Play Active `1.0.4-internal-vc5-uncap1tap`). Tip/script **6** / **1.0.5** (`ANDROID-VERSION-CODE-6`); **local vc6-iapBusy AAB built ~18:26 — NOT uploaded** (**NATIVE-VC6-LOCAL-AAB-SYNC**). Historical: **vc4** / 1.0.3; **vc3** / 1.0.2 |
...
- [x] `npm install` + `npm run aab:internal` (build:www → `cap sync` → Billing≥8 patch → versionCode **6** patch (`ANDROID-VERSION-CODE-6`; Active Play still vc5 until upload) → AdMob Manifest patch → `bundleRelease`)
...
- [ ] Play Console **upload** local `1.0.5-internal-vc6-iapBusy` AAB (path above; **NATIVE-VC6-LOCAL-AAB-SYNC**) — Active still vc5 until then
...
**DONE (repo + this env):** Capacitor deps, **REAL-ADMOB-IDS** (Android prod App/units + `USE_TEST_ADS=false`), billing skeleton, `index.html` script order, `build:www`, `cap add android` + `cap sync` with both plugins, idempotent Manifest / Billing≥8 / **versionCode 6** patch scripts (`ANDROID-VERSION-CODE-6`; Active Play still **vc5**), JDK 17 + ANDROID_HOME on this box, signing steps documented, signed internal AABs including **current Play Active** `1.0.4-internal-vc5-uncap1tap` (**UNCAP-ONE-TAP** / prod AdMob; **NATIVE-VC5-INTERNAL-SYNC**) plus historical vc4 / vc3 / vc2, **Play developer account APPROVED** (2026-09-21 Asia/Taipei). Tip main has **IAP-PURCHASE-BUSY** (#237) — **not** in Active vc5 AAB; **local vc6-iapBusy AAB built ~18:26** under `/workspace/colortube-artifacts/` (**NATIVE-VC6-LOCAL-AAB-SYNC**) — **ready for internal upload** as `1.0.5-internal-vc6-iapBusy`; Active remains vc5 until Console upload.
...
- Play rejects reuse of the same `versionCode` (prior uploads used **1**→**4**; **current** Play Active is still **5** / `1.0.4-internal-vc5-uncap1tap`). After `cap sync`, run `bash scripts/patch-android-version.sh` (hooked in `npm run aab:internal` after Billing-8, before AdMob) to set **versionCode 6** / **versionName 1.0.5** by default (override via `COLOR_TUBE_VERSION_CODE` / `COLOR_TUBE_VERSION_NAME`). Repo `package.json` `"version"` aligns to **1.0.5**. Accept gates: `ANDROID-VERSION-CODE-6` · `NATIVE-VC6-LOCAL-AAB-SYNC`. Does **not** claim MILLION_USER_BAR **#7 Pass**. Active track remains vc5 until the **already-built** local vc6-iapBusy AAB is uploaded to Play internal.
