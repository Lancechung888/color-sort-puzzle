...
| 6 | Meta 回來理由 | **Pass** | Daily 自適應＋首清 +40；連勝里程碑 Day **3/7/14**（幣＋提示）＋下一里程碑 HUD／首屏；首達 3★ **+22**；每 10 關全 3★ chapter chest（+80🪙＋1 hint）；關卡選單顯示缺星＋獎勵預告；**局中 status bar 即時 Moves/Par/★ 預算投影**（`track-perfect`／`track-good`／`track-ok`，掉軌 soft haptic＋`.track-drop`／Off N★；undo 回軌＋`.track-recover`／Back on N★；`#level-stars` 仍顯示 mastery best）；**Daily ready CTA juice**（start `#btn-start-daily.daily-ready` sky/cyan pulse ~1.6s + EN Ready badge + once-per-session arm haptic/SFX; done `.daily-done` + Done checkmark static; reduced-motion static） |
