...
| 3 | 倒水手感／15s 廣告 | **Partial** | WebAudio（pour/land/complete/uncap/win）＋**Capacitor Haptics**（native impact/notification）＋web `navigator.vibrate` fallback；開蓋 flip＋火花；**stream 對準目標口**（angle/length source→dest）＋**land 輕觸覺**（ImpactStyle LIGHT / vibrate ~10ms）。仍缺實機 15s UA 剪輯驗證 → 維持 Partial，未標 Pass |
...
2. **手感收滿 Pass** — stream→target＋land haptic 已上；仍差實機 15s UA 剪輯驗證（投放預算凍結期間可延後，閘門仍 Partial）。  
