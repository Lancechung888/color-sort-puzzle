#!/usr/bin/env bash
# Idempotent MainActivity file-access guard for local android/ (gitignored).
# Safe no-op if android/ missing or already patched. Exit 0 on success path.
# Stock WebView may allow file:// / file-URL access; ColorTube Sort serves
# https://localhost via Capacitor and needs no WebView filesystem access.
# Deny at WebSettings so mid-run cannot open file:// or escalate via file URLs
# (Play Data Safety / privacy). Distinct from CLEARTEXT / MIXED-CONTENT /
# GEOLOCATION-OFF. Does NOT touch setAllowContentAccess (Cap plugins).
set -euo pipefail
...
log() { echo "[patch-android-webview-file-access-off] $*"; }
...
if already:
    print(
        "[patch-android-webview-file-access-off] setAllowFileAccess(false) / "
        "FromFileURLs / UniversalAccess already present — ok."
    )
else:
    anchors = (
    ...
    for pattern, label in anchors:
        ...
        if match:
            ...
            changed = True
            print(
                f"[patch-android-webview-file-access-off] Inserted file-access-off after {label}."
            )
            break
    else:
...
assert re.search(
    r'webView\.getSettings\(\)\s*\.\s*setAllowFileAccess\s*\(\s*false\s*\)\s*;',
    check,
), "setAllowFileAccess(false) missing after patch"
assert re.search(
    r'webView\.getSettings\(\)\s*\.\s*setAllowFileAccessFromFileURLs\s*\(\s*false\s*\)\s*;',
    check,
), "setAllowFileAccessFromFileURLs(false) missing after patch"
assert re.search(
    r'webView\.getSettings\(\)\s*\.\s*setAllowUniversalAccessFromFileURLs\s*\(\s*false\s*\)\s*;',
    check,
), "setAllowUniversalAccessFromFileURLs(false) missing after patch"
print(
    "[patch-android-webview-file-access-off] Assert ok — file-access-off present."
)
PY
