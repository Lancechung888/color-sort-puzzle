...
> Honesty: Play Active is **`1.0.5-internal-vc6-iapBusy`** / **vc6** / **1.0.5**, containing **UNCAP-ONE-TAP** + **IAP-PURCHASE-BUSY** (Active **lacks** tip **AD-REWARD-UNAVAILABLE** / #244). The signed AAB was already built on this packaging box (2026-09-21 Asia/Taipei ~18:26): `/workspace/colortube-artifacts/ColorTubeSort-internal-20260921-1826-prodAdMob-vc6-iapBusy-release.aab` (+ `ColorTubeSort-internal-latest-release.aab` + `play-upload/ColorTubeSort-vc6-iapBusy.aab`) and uploaded to Play Active; **no AAB rebuild**. Tip `main` has **AD-REWARD-UNAVAILABLE**; **script default** is now **ANDROID-VERSION-CODE-7** (`versionCode` **7** / `versionName` **1.0.6`) for the *next* upload — Active track remains vc6 until that AAB is uploaded. Do **not** claim vc7 uploaded. **#7 Pass** (DEVICE-THREE-GREEN Seeker) unchanged. Markers: **NATIVE-VC6-INTERNAL-SYNC** · **NATIVE-VC6-LOCAL-AAB-SYNC**.
...
| Play Active `versionCode` 6 / `versionName` 1.0.5 (`ANDROID-VERSION-CODE-6`) | **UPLOADED current** (Play Active `1.0.5-internal-vc6-iapBusy`, **UNCAP-ONE-TAP** + **IAP-PURCHASE-BUSY**). Script default now **7** / **1.0.6** (`ANDROID-VERSION-CODE-7`) for next upload (tip **AD-REWARD-UNAVAILABLE**). Historical: **vc5** / 1.0.4; **vc4** / 1.0.3; **vc3** / 1.0.2 |
...
- [x] `npm install` + `npm run aab:internal` (build:www → `cap sync` → Billing≥8 patch → versionCode **7** patch (`ANDROID-VERSION-CODE-7`; Active Play still vc6 until upload) → AdMob Manifest patch → `bundleRelease`; existing signed AAB uploaded to Active; **no AAB rebuild**)
...
- [ ] Play Console **store listing** / open testing / production (internal testing **vc6-iapBusy** AAB is Active; testers configured — see **INTERNAL-TESTER-SYNC**; device three green lights **Pass** (DEVICE-THREE-GREEN Seeker))
...
**DONE (repo + this env):** Capacitor deps, **REAL-ADMOB-IDS** (Android prod App/units + `USE_TEST_ADS=false`), billing skeleton, `index.html` script order, `build:www`, `cap add android` + `cap sync` with both plugins, idempotent Manifest / Billing≥8 / **versionCode 7** patch scripts (`ANDROID-VERSION-CODE-7`; Active Play still **vc6**), JDK 17 + ANDROID_HOME on this box, signing steps documented, signed internal AABs including **current Play Active** `1.0.5-internal-vc6-iapBusy` (**UNCAP-ONE-TAP** + **IAP-PURCHASE-BUSY** / prod AdMob; **NATIVE-VC6-INTERNAL-SYNC**) plus historical vc5 / vc4 / vc3 / vc2, **Play developer account APPROVED** (2026-09-21 Asia/Taipei). The already-built **vc6-iapBusy AAB** under `/workspace/colortube-artifacts/` is uploaded as `1.0.5-internal-vc6-iapBusy` (**NATIVE-VC6-LOCAL-AAB-SYNC**); **no AAB rebuild**. Tip main has **AD-REWARD-UNAVAILABLE** (#244) — **not** in Active vc6 AAB; next upload uses script default **7** / **1.0.6**.
...
- Play rejects reuse of the same `versionCode` (prior uploads used **1**→**5**; **current** Play Active is still **6** / `1.0.5-internal-vc6-iapBusy`). After `cap sync`, run `bash scripts/patch-android-version.sh` (hooked in `npm run aab:internal` after Billing-8, before AdMob) to set **versionCode 7** / **versionName 1.0.6** by default (override via `COLOR_TUBE_VERSION_CODE` / `COLOR_TUBE_VERSION_NAME`). Repo `package.json` `"version"` aligns to **1.0.6**. Accept gates: `ANDROID-VERSION-CODE-7` · `NATIVE-PACK-READY-SYNC` · `NATIVE-VC6-INTERNAL-SYNC` · `NATIVE-VC6-LOCAL-AAB-SYNC`. **#7 Pass** is DEVICE-THREE-GREEN Seeker evidence (not this version bump). Active track remains vc6 until the next AAB (tip **AD-REWARD-UNAVAILABLE**) is uploaded as vc7. Do **not** claim vc7 uploaded. Active AAB contains **UNCAP-ONE-TAP** + **IAP-PURCHASE-BUSY** and **lacks** **AD-REWARD-UNAVAILABLE**; **no AAB rebuild**.
