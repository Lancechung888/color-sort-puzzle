...
- Play rejects reuse of the same `versionCode` (prior uploads used **1**→**5**; **current** Play Active is **6** / `1.0.5-internal-vc6-iapBusy`). After `cap sync`, run `bash scripts/patch-android-version.sh` (hooked in `npm run aab:internal` after Billing-8, before AdMob) to set **versionCode 6** / **versionName 1.0.5** by default (override via `COLOR_TUBE_VERSION_CODE` / `COLOR_TUBE_VERSION_NAME`). Repo `package.json` `"version"` aligns to **1.0.5**. Accept gates: `ANDROID-VERSION-CODE-6` · `NATIVE-VC6-INTERNAL-SYNC` · `NATIVE-VC6-LOCAL-AAB-SYNC`. **#7 Pass** is DEVICE-THREE-GREEN Seeker evidence (not this version bump). Active AAB contains **UNCAP-ONE-TAP** + **IAP-PURCHASE-BUSY**; **no AAB rebuild**.
