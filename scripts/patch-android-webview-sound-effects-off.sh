#!/usr/bin/env bash
# Idempotent MainActivity sound-effects guard for local android/ (gitignored).
# Safe no-op if android/ missing or already patched. Exit 0 on success path.
# After HAPTIC-OFF, taps can still trigger Android View system click sounds
# (View.playSoundEffect), which fight intentional WebAudio SFX mid-run.
# Distinct from ANDROID-WEBVIEW-HAPTIC-OFF (system vibrate) and from game
# playSfx() / WebAudio (still on via JS / Settings Sound toggle).
set -euo pipefail
...
log() { echo "[patch-android-webview-sound-effects-off] $*"; }
...
block = "      webView.setSoundEffectsEnabled(false);\n"

already = re.search(
    r'webView\.setSoundEffectsEnabled\s*\(\s*false\s*\)\s*;', raw
)

if already:
    print(
        "[patch-android-webview-sound-effects-off] setSoundEffectsEnabled(false) "
        "already present — ok."
    )
else:
    anchors = (
    ...
    for pattern, label in anchors:
        ...
        if match:
            ...
            changed = True
            print(
                f"[patch-android-webview-sound-effects-off] Inserted sound-effects-off after {label}."
            )
            break
    else:
...
assert re.search(
    r'webView\.setSoundEffectsEnabled\s*\(\s*false\s*\)\s*;', check
), "setSoundEffectsEnabled(false) missing after patch"
print(
    "[patch-android-webview-sound-effects-off] Assert ok — sound-effects-off present."
)
PY
