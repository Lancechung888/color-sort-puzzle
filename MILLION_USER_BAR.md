...
> **Play Console developer account APPROVED** (2026-09-21 Asia/Taipei). **REAL-ADMOB-IDS** wired Android prod units + `USE_TEST_ADS=false`. **#7 remains Fail** until device three green lights: (1) interstitial on device, (2) rewarded after full watch, (3) `remove_ads` purchase+restore (+ no mid-pour ads). Do **not** flip to Pass/Partial on repo wiring / AAB alone.
...
> **Play account approved** (2026-09-21 Asia/Taipei). **REAL-ADMOB-IDS** (same day): Android prod App ID + interstitial/rewarded wired + `USE_TEST_ADS=false`. **#7 still Fail** until device three green lights: (1) interstitial shows, (2) rewarded after **full watch**, (3) `remove_ads` purchase+restore. Do **not** mark #7 Pass/Partial on wiring or AAB alone.
...
| 7 | 真 AdMob＋remove_ads | **Fail** | **REAL-ADMOB-IDS**（2026-09-21 Asia/Taipei）：Android 正式 App ID `…~6670970617`＋interstitial `…/2731725604`＋rewarded `…/8768677032`；`USE_TEST_ADS=false`；`initializeForTesting=false`；`patch-android-admob.sh` 從 `capacitor.config.json` 注入（無殘留 sample `3940…`）。**仍 Fail** — 缺實機三綠燈：（1）interstitial 實機播出（2）rewarded **完整看完**發獎（3）`remove_ads` 購買＋還原。商店「去除廣告」→ **即將開放**，**不會**假授 `removeAds`（DEV flag 預設 OFF）。iOS 仍暫留 Google sample App ID。**未**標 Pass／Partial。 |
