...
| 包裝箱（JDK17＋SDK＋keystore＋已 `cap add`） | **已驗證**可產出 signed AAB（含現況 vc7） |
| Play internal 上傳 | **已上傳 current** vc7：`1.0.6-internal-vc7-adUnavailable`／versionCode **7**／versionName **1.0.6**，含 **UNCAP-ONE-TAP**＋**IAP-PURCHASE-BUSY**＋**AD-REWARD-UNAVAILABLE**（**NATIVE-VC7-INTERNAL-SYNC**）。無 AAB rebuild；建置來源為已建 AAB（**NATIVE-VC7-LOCAL-AAB-SYNC**）。歷史 vc6 superseded（**NATIVE-VC6-INTERNAL-SYNC**／**NATIVE-VC6-LOCAL-AAB-SYNC**） |
