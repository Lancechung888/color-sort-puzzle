...
| 9 | 穩定／ACCEPTANCE | **Partial** | `npm run accept` 自動化 **全綠**（關卡／經濟／P0①／測 ID 配線／倒水護欄／**SAVE-RECOVER**／**A11Y-TUBE**／**A11Y-ESC**／**SAVE-SANITIZE**）；**存檔** `sanitizeSave`＋`v2_bak`／legacy fallback＋quota retry；**鍵盤** tube `tabIndex`＋Enter/Space＋Escape dismiss overlays＋`:focus-visible`；**P0②③** 仍 Blocked → 未滿 Pass；無已知 P0 崩 |
...
3. **ACCEPTANCE → Pass** — `npm run accept` 已可重複跑且自動化項全綠（含 keyboard tube focus／Escape dismiss／save sanitize）；仍差 P0②③ 實機變現複驗後才能把 #9 從 Partial 拉滿。  
