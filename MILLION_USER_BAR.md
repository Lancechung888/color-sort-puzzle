...
（已補）**CAP-TEACH-ARM**已上 — L3 `teach:cap` 首分鐘揭蓋教學：`capTeachDone` **僅**真實 uncap 寫入；tip dismiss 只隱藏；load once-per-load `.lid-arm-nudge`＋`haptic('arm')`／`SFX.tap` @～300ms；EN tip 縮短 double-tap gold lid；accept `CAP-TEACH-ARM`；#1 維持 Partial；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
