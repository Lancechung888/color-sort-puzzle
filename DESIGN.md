...
- **Satisfying pours:** Lift, tilt, stream, splash particles, complete-tube glow + light screen shake — dopamine without complex systems. **INSTANT-CLARITY-P0:** V1 ≤2.9s start idle 擋→揭→倒 + demote Blocked→Lid + thick gold lid; C1「Uncap the gold lid. Then pour.」+ sub + Play; T1 L3 pain-then-joy (pour-capped block → then uncap; L1–2 unchanged; Skip V2). **INSTANT-CLARITY-MUTE-EVIDENCE** (2026-09-26): cold mute START clip + measured teachComplete **1740ms** / cycle **2900ms** under `/workspace/colortube-artifacts/gate1_*` (then HOOK-LOUD re-measure **1972ms**); historical Jev Partial pre-re-blind. **INSTANT-CLARITY-HOOK-LOUD** (2026-09-26 follow-up): enlarge start-hook + thick gold lid + strong glow 0–30% + Lid pill from t=0; teachComplete **~1972ms** / cycle **2900ms**; mute evidence re-capture with forced `prefers-reduced-motion: no-preference` + anim restart @t0 (t0/t1 must show gold lid). **INSTANT-CLARITY-GATE1-PASS** (2026-09-26): re-blind watchVideo + Jev Choice **Pass** conf **0.9** + measured 1972ms → **#1 Pass**; Score 3.14 Near Pass conf 0.79 caveat (agent blind ≠ paid human panel); **never fake #3 Pass**. **POUR-FEEL-MIDLAND:** board commits at mid-stream land (~250ms) so dest fill-rise plays under a live ~460ms stream (CSS `streamFall` synced); kills teleport liquid. **POUR-STREAM-THICK:** `.pour-stream` 12→18px + stronger glow + splash 6→7 (timing unchanged). **POUR-LAND-SPLASH:** splash + land SFX/haptic fire at LAND_MS≈250 with fill-rise (POUR_MS cleanup only). **POUR-STREAM-CONTINUITY:** fix aim sign (−atan2) so ribbon grows source→dest; streamFall full by ~40%; solid hex; dist×1.18; trail (keeps land splash). **#3 stays Partial** (web thicken ≠ device paid UA; UA frozen — do not invent clip Pass). Production / soft-launch forbidden.
...
**TODO / honesty (not ship-ready):** AdMob Capacitor plugin + Android **prod** App/unit IDs are wired (**REAL-ADMOB-IDS**; `USE_TEST_ADS=false`). Android **#7 Pass** via **DEVICE-THREE-GREEN** (Seeker 2026-09-21; interstitial + rewarded full-watch + remove_ads purchase+restore; see `docs/DEVICE_THREE_GREEN.md`). Shop「去除廣告」stays **Coming soon / needs store account** on web and does **not** set `removeAds` unless a real Billing success (DEV flag `localStorage.colorTubeSort_devIap=1` must stay OFF in uploaded AABs). iOS AdMob App ID still Google sample until an iOS app exists. Gate still **CLOSED**: MILLION_USER_BAR **#1 Pass** / **#3 Partial**; Production / soft-launch forbidden.
