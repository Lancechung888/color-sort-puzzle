...
- **Satisfying pours:** Lift, tilt, stream, splash particles, complete-tube glow + light screen shake — dopamine without complex systems. **INSTANT-CLARITY-P0:** V1 ≤2.9s start idle 擋→揭→倒 + demote Blocked→Lid + thick gold lid; C1「Uncap the gold lid. Then pour.」+ sub + Play; T1 L3 pain-then-joy (pour-capped block → then uncap; L1–2 unchanged; Skip V2). **#1 stays Partial** until external mute stranger 3s evidence — never fake Pass. **INSTANT-CLARITY-MUTE-EVIDENCE** (2026-09-26): cold mute START clip + measured teachComplete **1740ms** / cycle **2900ms** under `/workspace/colortube-artifacts/gate1_*`; blind watchVideo still pending from executor (no Task) + Jev Choice=Partial — **keep Partial**, never fake Pass. **INSTANT-CLARITY-HOOK-LOUD** (2026-09-26 follow-up): enlarge start-hook + thick gold lid + strong glow 0–30% + Lid pill from t=0; teachComplete **~1972ms** / cycle **2900ms**; mute evidence re-capture with forced `prefers-reduced-motion: no-preference` + anim restart @t0 (t0/t1 must show gold lid); **#1 stays Partial** awaiting re-blind — never fake Pass. **POUR-FEEL-MIDLAND:** board commits at mid-stream land (~250ms) so dest fill-rise plays under a live ~460ms stream (CSS `streamFall` synced); kills teleport liquid. **POUR-STREAM-THICK:** `.pour-stream` 12→18px + stronger glow + splash 6→7 (timing unchanged). **#3 stays Partial** (web thicken ≠ device paid UA; UA frozen — do not invent clip Pass). Production / soft-launch forbidden.
