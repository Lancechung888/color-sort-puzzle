...
| 9 | 穩定／ACCEPTANCE | **Partial** | `npm run accept` 自動化 **全綠**（關卡／經濟／P0①／測 ID 配線／倒水護欄／**SAVE-RECOVER**／**A11Y-TUBE**／**A11Y-ESC**／**A11Y-FOCUS**／**A11Y-DIALOG**／**SAVE-SANITIZE**）；**存檔** `sanitizeSave`＋`v2_bak`／legacy fallback＋quota retry（corrupt `stars`/typed fields 不再啟動崩）；**鍵盤** tube Tab／Enter／Space＋Escape dismiss＋**overlay focus move-in／restore**（`focusOverlayPrimary`／depth）；**P0②③** 仍 Blocked → 未滿 Pass；無已知 P0 崩 |
...
3. **ACCEPTANCE → Pass** — `npm run accept` 已可重複跑且自動化項全綠（含 keyboard tube focus／Escape dismiss／**overlay focus restore**／dialog labelledby／save sanitize）；仍差 P0②③ 實機變現複驗後才能把 #9 從 Partial 拉滿。  
