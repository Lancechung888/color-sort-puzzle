...
> **Play Console developer account APPROVED** (2026-09-21 Asia/Taipei). **REAL-ADMOB-IDS** wired Android prod units + `USE_TEST_ADS=false`. **#7 Pass** on **DEVICE-THREE-GREEN** Seeker evidence (2026-09-21 ~19:28–19:49 Asia/Taipei): interstitial + rewarded full-watch + `remove_ads` purchase+restore (`GPA.3327-2483-8031-49087`). Mid-pour ads still guarded in source. See `docs/DEVICE_THREE_GREEN.md`. **Production / soft-launch still forbidden** while #3 Partial.
**EN:** Live AdMob + real remove_ads IAP verified on device; never interrupt mid-pour.
...
> **Play account approved** (2026-09-21 Asia/Taipei). **REAL-ADMOB-IDS** wired. **#7 Pass** via **DEVICE-THREE-GREEN** on Seeker (2026-09-21 ~19:28–19:49 Asia/Taipei) — interstitial + rewarded full-watch + `remove_ads` purchase+restore (`GPA.3327-2483-8031-49087`); see `docs/DEVICE_THREE_GREEN.md`. Mid-pour ads still guarded. **#1 Pass**（INSTANT-CLARITY-GATE1-PASS）；**#3 stays Partial**. **Production / soft-launch still forbidden**. Gate CLOSED until all 1–9 Pass.

依目前程式／資產／實機證據閱讀（非願望清單）。**禁止 soft-launch**；**不宣稱 ship-ready**。

| # | 項目 | 結果 | 現況依據（代碼事實） |
|---|------|------|----------------------|
| 1 | 3秒看懂＋想點 | **Pass** | 開始屏 `.start-hook`＋短 lead；ICON A 1024（**原生 launcher／splash 已套 finals**，非 Capacitor 預設）；**首倒成功拍**；**選管後合法目標管 `.pour-target` 脈衝高亮**（持液即知往哪倒）；**非法倒水原因 toast**（`Tube full`／`Colors don't match`，蓋管另 toast）；**INSTANT-CLARITY-P0**（**V1** start idle **≤2.9s** 擋→揭→倒 silent loop＋demote `Blocked`→`Lid`＋thick gold lid；**C1** lead「Uncap the gold lid. Then pour.」＋short sub＋CTA Play；**T1** L3 pain-then-joy：先嘗試蓋擋 → toast「Lid blocks pour」→ tip Uncap＋soft-arm；L1–2 不變；Skip V2）；**start-hook USP**（金蓋擋倒 → quiet `Lid` pill＋shake → uncap → pour → sorted；`prefers-reduced-motion` 靜態蓋＋badge）；**Continue · Level N** start primary CTA（`#btn-start.play-continue` soft warm gold/peach pulse ~1.6s＋once-per-session soft arm haptic/SFX；fresh → Play；loads labeled level；`prefers-reduced-motion` 靜態）；**Levels continue soft-arm**（`#levels-overlay` 當前／frontier `.level-cell.level-continue-arm` warm gold/peach pulse ~1.55s＋`Go` badge＋once-per-open `haptic('arm')`／`SFX.tap` @300ms；daily 時退回 frontier；關 overlay 清 timer；`prefers-reduced-motion` 靜態金邊）。**INSTANT-CLARITY-MUTE-EVIDENCE**（2026-09-26）：cold mute START clip＋measure teachComplete **1740ms**／cycle **2900ms**；Jev Partial；**仍缺** blind watchVideo stranger ID（executor 無 Task）／鐵證「想點」→ **維持 Partial，未標 Pass**。**INSTANT-CLARITY-HOOK-LOUD**（2026-09-26 follow-up）：enlarge `.start-hook`＋thick gold lid＋strong glow 0–30%＋Lid pill from t=0；teachComplete **~1972ms**／cycle **2900ms**；mute evidence re-capture forced `prefers-reduced-motion: no-preference`＋anim restart @t0。**INSTANT-CLARITY-GATE1-PASS**（2026-09-26）：HOOK-LOUD＋mute clip `/workspace/colortube-artifacts/gate1_instant_clarity_mute_start_evidence.mp4` teachComplete **1972ms**≤3s（gold lid t0/t1）；re-blind watchVideo（zero coaching）見 gold lid＋Lid→uncap→pour／first 3s=color-sort＋gold lids blockers／tap Play（in-level tap gold lid）；Jev `jev-1.13.0` `gate1_status` Choice **Pass** conf **0.9**（Pass 0.94）；`mute_stranger_clarity` Score **3.14** Near Pass conf **0.79**；`blind_ids_usp` Noul **0.93**；caveat Score conf 0.79／agent blind ≠ paid human stranger panel — still **#1 → Pass** per Choice≥0.85＋measured＋blind ID。**Do NOT mark #3 Pass.** **Production／soft-launch 仍禁止** |
...
（已補）**POUR-STREAM-CONTINUITY**已上 — 修正 aim 符號 `angleDeg=-atan2(dx,dy)`（舊符號 ribbon 往回長到源管＝mute stub）；`streamFall` ~40% 達全長＋solid SOLIDS hex＋dist×1.18＋7 trail；KEEP LAND_MS splash／THICK 18px／MIDLAND；accept `POUR-STREAM-CONTINUITY`；**#3 仍 Partial**（web stream continuity ≠ device paid UA；UA 凍結 — **永不假標 Pass**）；閘門仍 **8 Pass／1 Partial／0 Fail**；**無** soft-arm／claim-juice／HUD pulse。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**

（已補）**INSTANT-CLARITY-GATE1-PASS**已上 — honesty mark **#1 Instant clarity = Pass**：HOOK-LOUD＋mute START clip teachComplete **1972ms**≤3s；re-blind watchVideo（zero coaching）IDs gold lid＋Lid→uncap→pour；Jev `jev-1.13.0` Choice **Pass** conf **0.9**（Pass 0.94）；Score 3.14 Near Pass conf 0.79 caveat（agent blind ≠ paid human panel）；accept `INSTANT-CLARITY-GATE1-PASS`；**#3 仍 Partial**（**不標 #3 Pass**）；閘門 **8 Pass／1 Partial／0 Fail** 仍關閉（需 1–9 全 Pass）；**無** soft-arm／claim-juice／HUD pulse。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**

（已補）**INSTANT-CLARITY-HOOK-LOUD**已上 — enlarge start-hook／thick gold lid／strong glow 0–30%／Lid pill @t=0；teachComplete **~1972ms**／cycle **2900ms**；mute clip＋t0–t3 re-capture（forced motion；t0/t1 must show gold lid）；accept `INSTANT-CLARITY-HOOK-LOUD`；**#1 仍 Partial**（awaiting re-blind — **永不假標 Pass**）；**#3 仍 Partial**；閘門仍 **7 Pass／2 Partial／0 Fail**；**無** soft-arm／claim-juice／HUD pulse。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**

（已補）**INSTANT-CLARITY-MUTE-EVIDENCE**已上 — cold mute START `gate1_instant_clarity_mute_start_evidence.mp4`（7s／1080×1920／silent）＋t0–t3＋measure teachComplete **1740ms**／cycle **2900ms**；Jev Choice=**Partial**；blind watchVideo pending；accept `INSTANT-CLARITY-MUTE-EVIDENCE`；**#1 仍 Partial**（永不假標 Pass）；**#3 仍 Partial**；閘門仍 **7 Pass／2 Partial／0 Fail**；**無** soft-arm／claim-juice／HUD pulse。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**

（已補）**POUR-STREAM-THICK**已上 — `.pour-stream` **12→18px**／radius 9／較強 glow；JS center offset **7→9**；splash particle **6→7**＋win/complete/first 略濃 splash；KEEP mid-land timing；accept `POUR-STREAM-THICK`；**#3 仍 Partial**（web 加粗證據 clip ≠ device paid UA；UA 凍結無預算 — **永不假標 Pass**）；**#1 仍 Partial**；閘門仍 **7 Pass／2 Partial／0 Fail**；**無** soft-arm／claim-juice／HUD pulse。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**

（已補）**INSTANT-CLARITY-P0**（V1＋C1＋T1）已上 — start idle **≤2.9s** silent 擋→揭→倒；demote `Blocked`→`Lid`；thick gold lid；lead「Uncap the gold lid. Then pour.」＋short sub＋CTA Play；L3 teach pain-then-joy（先蓋擋 toast → 再 Uncap tip＋arm；**不改 L1–2**；Skip V2）；accept `INSTANT-CLARITY-P0`；**#1 仍 Partial**（缺外部 mute stranger 3s 證據 — **永不假標 Pass**）；**#3 仍 Partial**（POUR-FEEL-MIDLAND 已在 main；15s UA 剪輯仍缺；UA 凍結無預算 — **不發明 clip Pass**）；閘門仍 **7 Pass／2 Partial／0 Fail**；**無** soft-arm／claim-juice／HUD pulse。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**
...
（已補）**DEVICE-THREE-GREEN**已上 — 2026-09-21 ~19:28–19:49 Asia/Taipei 助理於 Seeker（`SM02G4061932272`）ADB self-verify：**#7 Pass**（interstitial／rewarded full-watch／`remove_ads` purchase+restore order `GPA.3327-2483-8031-49087`；vc6／1.0.5；`hanwen16888@gmail.com`）；證據 `docs/DEVICE_THREE_GREEN.md`；ACCEPTANCE P0②③ → **Pass**；accept P0-2／P0-3／M-IAP 改證據 Pass（目標 **0 Blocked**）；**#9 → Pass**；閘門 **7 Pass／2 Partial／0 Fail** 仍關閉（#1／#3 Partial）。Mid-pour 護欄仍在源碼。**無** soft-arm／claim-juice／HUD pulse。**未**把 #1／#3 標 Pass。**Production／soft-launch 仍禁止**。**不宣稱 ship-ready。**
...
（已補）**NATIVE-VC6-INTERNAL-SYNC**／**PLAY-PASTE-VC6-SYNC**已上 — Play **internal testing** 現況對齊為 `1.0.5-internal-vc6-iapBusy`／versionCode **6**／versionName **1.0.5**／Billing≥8／prod AdMob／**UNCAP-ONE-TAP**＋**IAP-PURCHASE-BUSY**；歷史 vc5／vc4／vc3／vc2 標 superseded；accept 兩項；**#7 仍 Fail**（缺實機三綠燈）；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**無** soft-arm／claim-juice／HUD pulse。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**SHOP theme 金幣一鍵解鎖**已上 — Neon／Cat theme 金幣 CTA 一鍵 `spendCoins`＋`unlockTheme`；hints-pack／undo-level 仍兩次確認；`SHOP-SPEND-CONFIRM` accept 已改；**無** soft-arm；#9 維持 Partial（仍差 P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。
...
（已補）**商店大額金幣消費兩次確認**已上 — hints-pack（120）／undo（80）金幣 CTA 先 toast「Tap again to spend N🪙」～2s，再點才扣幣；**theme（280）改一鍵解鎖**（同餘額／toast）；小額 hint 25🪙 不需確認；關店／換關清 pending；**無** soft-arm CSS／pulse；#9 維持 Partial（仍差 P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。
...
（已補）**ANDROID-FORCE-DARK**已上 — `<application>`＋`AppTheme`／`NoActionBar`／`NoActionBarLaunch` `android:forceDarkAllowed="false"`（拒絕 Android 10+ Force Dark 改寫 brand-dark `#1a1a2e` tubes／juice／HUD；`android/` 仍 gitignore）；`scripts/patch-android-force-dark.sh`＋`aab:internal` hook（is-game 之後）＋`native-templates/android/README.md` §2k；accept `ANDROID-FORCE-DARK`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（仍差 P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**ANDROID-CUTOUT**已上 — `AppTheme`／`NoActionBar`／`NoActionBarLaunch` `android:windowLayoutInDisplayCutoutMode=shortEdges`（WebView 可畫進 notch／cutout，既有 CSS `env(safe-area-inset-*)` 才有非零 insets；`android/` 仍 gitignore）；`scripts/patch-android-cutout.sh`＋`aab:internal` hook（config-changes 之後）＋`native-templates/android/README.md` §2m；accept `ANDROID-CUTOUT`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**

（已補）**CAP-APP-BACK**已上 — `package.json` 補 `@capacitor/app` ^6.0.3（對齊 `@capacitor/core` ^6.2／`@capacitor/haptics` ^6.x）；`bindSystemBack` 既有 `App.addListener('backButton')` 現可真正註冊（先前缺 dep 時硬體／手勢返回可能直接 exit 跳過 BACK-NAV）；本機 `npx cap sync` 已見 plugin；`android/` 仍 gitignore；accept `CAP-APP-BACK`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**

（已補）**CAP-APP-STATE**已上 — `bindAppState`＋`App.addListener('appStateChange')`：背景 flush mid-run draft／`clearPendingUncap`／釋 wake；回前景重同步 wake lock／native keep-screen-on（補 visibilitychange 在部分 Android WebView 不穩）；沿用既有 `@capacitor/app` ^6；accept `CAP-APP-STATE`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**ANDROID-WEBVIEW-GEOLOCATION-OFF**已上 — `MainActivity` `webView.getSettings().setGeolocationEnabled(false)`（拒絕 WebView 地理位置；本遊戲不收集位置，對齊 Data Safety／隱私；`android/` 仍 gitignore）；順帶還原 keep-awake 模板缺漏的 `setSoundEffectsEnabled(false)`＋`setMediaPlaybackRequiresUserGesture(false)` 實行行；`scripts/patch-android-webview-geolocation-off.sh`＋`aab:internal` hook（mixed-content 之後、icons 之前）＋`native-templates/android/README.md` §2y；accept `ANDROID-WEBVIEW-GEOLOCATION-OFF`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
（已補）**ANDROID-WEBVIEW-FILE-ACCESS-OFF**已上 — `MainActivity` `setAllowFileAccess(false)`＋`setAllowFileAccessFromFileURLs(false)`＋`setAllowUniversalAccessFromFileURLs(false)`（拒絕 WebView `file://`／file-URL；Capacitor 走 https://localhost；不碰 `setAllowContentAccess`；對齊 Data Safety／隱私；`android/` 仍 gitignore）；`scripts/patch-android-webview-file-access-off.sh`＋`aab:internal` hook（geolocation-off 之後、icons 之前）＋`native-templates/android/README.md` §2z；accept `ANDROID-WEBVIEW-FILE-ACCESS-OFF`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
（已補）**ANDROID-WEBVIEW-JS-WINDOWS-OFF**已上 — `MainActivity` `setSupportMultipleWindows(false)`＋`setJavaScriptCanOpenWindowsAutomatically(false)`（拒絕 WebView 多視窗／`window.open` 彈窗；單 WebView hybrid-casual；不關 JavaScript 本身；不碰 DomStorage／content access／cookies；`android/` 仍 gitignore）；`scripts/patch-android-webview-js-windows-off.sh`＋`aab:internal` hook（file-access-off 之後、icons 之前）＋`native-templates/android/README.md` §2aa；accept `ANDROID-WEBVIEW-JS-WINDOWS-OFF`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
（已補）**ANDROID-WEBVIEW-SAFE-BROWSING**已上 — `MainActivity` `setSafeBrowsingEnabled(true)`（啟用 Android WebView Safe Browsing，阻擋釣魚／已知惡意 URL；API 26+；不關 JavaScript；不碰 DomStorage／content access／cookies；`android/` 仍 gitignore）；`scripts/patch-android-webview-safe-browsing.sh`＋`aab:internal` hook（js-windows-off 之後、icons 之前）＋`native-templates/android/README.md` §2ab；accept `ANDROID-WEBVIEW-SAFE-BROWSING`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
（已補）**ANDROID-WEBVIEW-DATABASE-OFF**已上 — `MainActivity` `setDatabaseEnabled(false)`（拒絕 deprecated Web SQL／WebDatabase；進度走 DomStorage／localStorage，**不**關 DomStorage；不碰 cookies／content access／JavaScript；異於 SAFE-BROWSING；`android/` 仍 gitignore）；`scripts/patch-android-webview-database-off.sh`＋`aab:internal` hook（safe-browsing 之後、icons 之前）＋`native-templates/android/README.md` §2ac；accept `ANDROID-WEBVIEW-DATABASE-OFF`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
（已補）**ANDROID-WEBVIEW-ALGORITHMIC-DARK-OFF**已上 — `MainActivity` API 33+ `setAlgorithmicDarkeningAllowed(false)`（拒絕 WebView 演算法變暗 brand-dark `#1a1a2e` tubes／juice／HUD；異於 ANDROID-FORCE-DARK 的 theme／application `forceDarkAllowed`；不碰 DomStorage／cookies／content access／JavaScript；`android/` 仍 gitignore）；`scripts/patch-android-webview-algorithmic-dark-off.sh`＋`aab:internal` hook（database-off 之後、icons 之前）＋`native-templates/android/README.md` §2ad；accept `ANDROID-WEBVIEW-ALGORITHMIC-DARK-OFF`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**ANDROID-WEBVIEW-DOM-STORAGE-ON**已上 — `MainActivity` `setDomStorageEnabled(true)`（明確啟用 DomStorage／localStorage，避免 OEM WebView 預設翻轉導致存檔靜默失效；異於 DATABASE-OFF 拒絕 Web SQL——DomStorage 必須維持 ON；不啟用 Web SQL／`setDatabaseEnabled`；不碰 cookies／content access／JavaScript／AdMob；`android/` 仍 gitignore）；`scripts/patch-android-webview-dom-storage-on.sh`＋`aab:internal` hook（debug-off 之後、icons 之前）＋`native-templates/android/README.md` §2af；accept `ANDROID-WEBVIEW-DOM-STORAGE-ON`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**FONT-PRELOAD**已上 — 可玩端 `index.html` 在 stylesheet 前 `rel=preload` Noto Sans latin **700／800／900** woff2（`as=font`／`type=font/woff2`／`crossorigin`）；首屏 brand／CTA 字重不需等 CSS `@font-face` 發現；`sync-www` → www／docs/play；accept `FONT-PRELOAD`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
（已補）**COLOR-SCHEME-DARK**已上 — `index.html`／`docs/index.html` `<meta name="color-scheme" content="dark">`＋`assets/css/style.css` `:root`／`html, body` `color-scheme: dark`（品牌暗底 `#1a1a2e`；不反轉調色盤）；`sync-www` → www／docs/play；accept `COLOR-SCHEME-DARK`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
（已補）**SCRIPT-PRELOAD**已上 — 可玩端 `index.html` 在 stylesheet 前 `rel=preload` 關鍵玩法腳本 `assets/js/levels.js`＋`assets/js/game.js`（`as=script`；同源無需 crossorigin；不 preload ads／billing／analytics）；與 CSS／字體並行下載以改善 TTI／Instant 清晰度（#1 仍 Partial）；`sync-www` → www／docs/play；accept `SCRIPT-PRELOAD`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
（已補）**INLINE-CRITICAL-BG**已上 — 可玩端 `index.html` 在 font／script preload 前 inline `html, body { background:#1a1a2e; color:#f5f5f7 }`，避免 CSS 尚未到達前白閃（#1 首屏）；`sync-www` → www／docs/play；accept `INLINE-CRITICAL-BG`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**SCRIPT-ORDER**已上 — 可玩端 `index.html` script 改為 `levels.js`→`game.js` 再 ads／billing／analytics（sync；monetization 不再擋在 levels 與 game 之間）；ads／billing 註解改為可在 game 後載入；`sync-www` → www／docs/play；accept `SCRIPT-ORDER`；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**CAP-TEACH-ARM**已上 — L3 `teach:cap` 首分鐘揭蓋教學：`capTeachDone` **僅**真實 uncap 寫入；tip dismiss 只隱藏；load once-per-load `.lid-arm-nudge`＋`haptic('arm')`／`SFX.tap` @～300ms；EN tip 改 Tap the gold lid（UNCAP-ONE-TAP）；accept `CAP-TEACH-ARM`；#1 維持 Partial；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**CRASH-GUARD**已上 — `window` `error`／`unhandledrejection` 一次／session：flush `persist`＋mid-run `persistRunDraft`（`activeOrStoredProgressDraft`／`draftHasProgress`）；toast「Something went wrong — progress was saved」；`trackEvent('client_error')`；swallow handler errors；不 preventDefault（DevTools 仍可用）；`sync-www` → www／docs/play；accept `CRASH-GUARD`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
（已補）**APPLE-SPLASH-MORE**已上 — 補 7 張 portrait `apple-touch-startup-image`：640×1136（SE1／5）／1242×2208（6+／7+／8+）／1125×2436（X／XS／11 Pro）／1242×2688（XS Max／11 Pro Max）／1080×2340（12／13 mini）／1536×2048（iPad 9.7）／2048×2732（iPad Pro 12.9）；品牌底 `#1a1a2e`＋ICON A；`sync-www` → www／docs/play；**未**進 sw PRECACHE；accept `APPLE-SPLASH-MORE`；**無** soft-arm／claim-juice／HUD pulse；閘門仍 5 Pass／3 Partial／1 Fail（#7 AdMob）；#9 維持 Partial（P0②③）。**未**把 #1／#3／#7／#9 標新 Pass。**不宣稱 ship-ready。**
...
4. **首屏／Icon「想點」** — finals icon／start-hook（**含蓋擋→揭蓋→倒** twist）／首倒成功拍／**選管合法目標高亮**／**非法倒水原因 toast**／**Continue · Level N** primary CTA（soft arm）／**Levels continue soft-arm**（frontier／current `Go` cell）／**CAP-TEACH-ARM**（L3 真實 uncap 才標 teach done＋load soft lid-arm；非 tip dismiss）已上；仍缺外部 3s 競品對照實測（#1 維持 Partial）。
