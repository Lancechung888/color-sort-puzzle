#!/usr/bin/env bash
# Idempotent MainActivity DomStorage / localStorage enable for local android/ (gitignored).
# Safe no-op if android/ missing or already patched. Exit 0 on success path.
# Explicitly set DomStorageEnabled(true) so OEM WebView defaults cannot silently
# break localStorage / save progress. Distinct from ANDROID-WEBVIEW-DATABASE-OFF
# (that denies deprecated Web SQL / WebDatabase — DomStorage must stay ON).
# Does NOT enable Web SQL / setDatabaseEnabled. Does NOT touch cookies /
# setAllowContentAccess / JavaScript / AdMob.
set -euo pipefail
...
log() { echo "[patch-android-webview-dom-storage-on] $*"; }
...
if already:
    print(
        "[patch-android-webview-dom-storage-on] "
        "setDomStorageEnabled(true) already present — ok."
    )
else:
    # Prefer after debug-off, then cascading older anchors.
    anchors = (
        (
            re.compile(
    ...
    for pattern, label in anchors:
        ...
        if match:
            ...
            changed = True
            print(
                f"[patch-android-webview-dom-storage-on] "
                f"Inserted dom-storage-on after {label}."
            )
            break
    else:
...
assert re.search(
    r'webView\.getSettings\(\)\s*\.\s*setDomStorageEnabled\s*\(\s*true\s*\)\s*;',
    check,
), "setDomStorageEnabled(true) missing after patch"
print(
    "[patch-android-webview-dom-storage-on] Assert ok — dom-storage-on present."
)
PY
