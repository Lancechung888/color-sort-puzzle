...
>
> Marker: **NATIVE-ACCEPTANCE-VC5-SYNC** · **NATIVE-VC6-LOCAL-AAB-SYNC**（對齊 `docs/NATIVE_PACK_READY.md`／`docs/PLAY_CONSOLE_PASTE.md` 的 **NATIVE-VC5-INTERNAL-SYNC**／**PLAY-PASTE-VC5-SYNC**）。
...
| Active AAB | `1.0.4-internal-vc5-uncap1tap`／vc5／1.0.4／Billing≥8／prod AdMob／**UNCAP-ONE-TAP**；**不含** **IAP-PURCHASE-BUSY**。本機已建 **vc6-iapBusy** AAB（~18:26；`…-1826-prodAdMob-vc6-iapBusy-release.aab`／`play-upload/ColorTubeSort-vc6-iapBusy.aab`）**尚未上傳**（**NATIVE-VC6-LOCAL-AAB-SYNC**） |
...
| Play internal 上傳 | **已上傳 current** vc5（見 `NATIVE_PACK_READY`）；本機 **vc6-iapBusy** AAB 已建（~18:26）**尚未上傳**；上傳後才會把 IAP-PURCHASE-BUSY 打進 Active（**NATIVE-VC6-LOCAL-AAB-SYNC**） |
...
- [x] **versionCode 腳本**：Active 仍為 **5／1.0.4**；`scripts/patch-android-version.sh` 預設 **6／1.0.5**（`ANDROID-VERSION-CODE-6`）；本機已產出 signed **vc6-iapBusy** AAB（~18:26；**NATIVE-VC6-LOCAL-AAB-SYNC**；尚未上傳）
...
- [ ] **實機三綠燈** → 才能把 MILLION_USER_BAR **#7** 從 Fail 挪開：  
  ...
  (2) rewarded **完整看完**發獎  
  (3) `remove_ads` 購買＋還原  
- [ ] 上傳本機已建內測 AAB：**versionCode 6**／**1.0.5**／`1.0.5-internal-vc6-iapBusy`（`/workspace/colortube-artifacts/ColorTubeSort-internal-20260921-1826-prodAdMob-vc6-iapBusy-release.aab`；**NATIVE-VC6-LOCAL-AAB-SYNC**）— Active 仍 vc5 直到 Console 上傳  
