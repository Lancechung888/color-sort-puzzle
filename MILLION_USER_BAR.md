...
| 3 | 倒水手感／15s 廣告 | **Partial** | WebAudio（pour/land/complete/uncap/win）＋**Capacitor Haptics**＋vibrate fallback；開蓋 flip＋火花；stream 對準目標口＋land 觸覺；**dest fill-rise**（新層 `land-rise` + glass `dest-receive`）。仍缺實機 15s UA 剪輯驗證 → 維持 Partial，未標 Pass |
...
2. **手感收滿 Pass** — stream→target＋land haptic＋dest fill-rise 已上；仍差實機 15s UA 剪輯驗證（投放預算凍結期間可延後，閘門仍 Partial）。  
